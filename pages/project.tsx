      ...
      <Box>
        ...
        <ProjectDiv>
          {ProjectDummy.map((v, i) => {
            if (
              !selectTagList.find((findValue) => v.tag.includes(findValue)) &&
...
export const ProjectDummy = [
  {
    ...
    depscription: () => (
      <>
        <p>
          5명의 팀원과 같이 협업을 하며 팀 프로젝트를 진행하는 부분에 대해 많이
          배울 수 있엇습니다. 프로젝트 진행 방식에 대해 많은 것을 배웠고,어려운
          부분이 있으면 소통을 하며 학습할 수 있엇습니다.
        </p>
        <br />
        <p>
          프론트엔드 빠른 개발을 위해 부트스트랩 프레임 워크를 이용하였으며,
          에어 부산의 디자인을 벤치마킹하여 깔끔한 디자인으로 작업하였습니다.
        </p>
        <br />
        <p>
          백엔드의 경우 MVC2 패턴을 사용하여 클라이언트의 요청,응답처리 및
          비즈니스 로직 처리부분을 모듈화 하여 유지보수 및 확장성에 용이하게
          작업을 진행하였습니다. 공공 빅데이터에서 국내 항공 정보 API를 이용해
          실제 항공 정보를 기반으로 예매가 가능하도록 작업 하였습니다. 해당
          API에서 국외데이터가 나오지 않아 항공정보포털시스템 사이트 크롤링을
          통해 국외 정보도 지원 하였습니다.
        </p>
        <br />
        <p>
          소스 관리 부분에선 Git을 사용 하여 관리하였으며, 당시엔 Git사용에
          익숙하지 않아 각자의 Branch에서 작업을 진행하였습니다. 작업 완료 후
          Main Branch에는 제가 직접 병합하는 식으로 작업을 진행했습니다.
        </p>
      </>
    ),
  },
  {
    ...
    depscription: () => (
      <>
        ...
        <p>
          소스 관리 부분에선 Git을 사용 하여 관리하였으며, 당시엔 Git사용에
          미숙함이 있어 각자의 Branch에서 작업을 진행하였습니다. 작업 완료 후
          Main Branch에는 제가 직접 병합하는 식으로 작업을 진행했습니다.
        </p>
      </>
    ),
  },
  {
    ...
    depscription: () => (
      <>
        <p>
          백엔드와 첫 협업을 한 프로젝트입니다. 소통을 하면서 커뮤니케이션
          능력이 얼마나 중요한지 깨닳게 되는 프로젝트 였습니다. React-Native를
          처음 다루는 프로젝트여서
        </p>
        <br />
        <p>
